...
class Optimize(object):
    ...
    def __init__(
        ...
        if not wandb_project_name:
            if self.use_turbo:
                if self.moss23_baseline:
                    wandb_project_name = f"run-aabo-moss-turbo-{task_id}-{acq_name}-{self.p_outliers}"    
                else:
                    wandb_project_name = f"run-aabo-turbo-{task_id}-{acq_name}-{self.p_outliers}"
            
            else:
                if self.moss23_baseline:
                    wandb_project_name = f"run-aabo-moss-{task_id}-{acq_name}-{self.p_outliers}"
                else:
                    wandb_project_name = f"run-aabo-{task_id}-{acq_name}-{self.p_outliers}"
        ...
        self.normed_train_y = self.objective.inject_contamination(self.normed_train_y, self.normed_train_y_std, outliers_noise, outliers_mask, low=self.lb_outliers, high=self.ub_outliers)
        ...
        while self.objective.num_calls < self.max_n_oracle_calls:    
            ...
            if self.exact_gp_baseline:
                ...
                if self.rcsvgp_baseline:
                    ...
                    likelihood = self.model.likelihood.to(self.device)
                    inducing_points = self.model.inducing_points
                    weight_function = StandardWeight(likelihood.noise)
                    learn_inducing_locations = self.model.learn_inducing_locations
                    ...
                    model_mll = RCSVGPVariationalELBO(likelihood, self.model)
            ...
            noise_y_next, noise_idx_y_next, noise_mask_y_next = self.objective.asymmetric_contamination(normed_y_next, seed=n_calls_, probability=self.p_outliers)
