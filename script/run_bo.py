...
from utils.weight_function import IMQ, get_soft_threshold
...
class Optimize(object):
    ...
    def __init__(
        ...
        bsz: int=1, 
        train_bsz: int=20,
        num_initialization_points: int=110, 
        lr: float=0.01,
        x_next_lr: float=0.001,
        acq_func: str="ei",
        n_update_epochs: int=5,
        n_inducing_pts: int=10, 
        grad_clip: float=2.0,
        eulbo: bool=True,
        use_turbo=False,
        use_kg=False,
        ...
        if not wandb_project_name:
            if not self.alternate_eulbo_updates:
                wandb_project_name = f"run-aabo-joint-{task_id}"
            else:
                wandb_project_name = f"run-aabo-{task_id}-{self.p_outliers}"
        ...
        outliers_noise, self.outliers_idx, outliers_mask = self.objective.asymmetric_contamination(self.normed_train_y, probability=self.p_outliers)
        self.normed_train_y_std = self.normed_train_y.std()
        self.normed_train_y = self.objective.inject_contamination(self.normed_train_y, self.normed_train_y_std, outliers_noise, outliers_mask)
        ...
        if not self.exact_gp_baseline:
            ...
            if self.rcsvgp_baseline:
                ...
                if informative_mean and len(self.outliers_idx) > 0:
                    ...
                    self.Y_corrections = simulate_expert_correction(x_outliers, 
                                                            self.objective,
                                                            sigma_sq=sigma_sq)
                    shifting_const = 0.0
                    mean_module = InformativeMeanPrior(
                        self.train_y.to(self.device),
                        self.outliers_idx,
                ...
                beta = likelihood.noise / np.sqrt(2)
                C = torch.tensor(1.0)
                ...
                self.model = RCSVGP(
                    ...
                    self.normed_train_y.to(self.device),
                    mean_module,
                    covar_module,
                ...
                if informative_mean and len(self.outliers_idx) > 0:
                    x_outliers = self.train_x[self.outliers_idx]
                    self.Y_corrections = simulate_expert_correction(x_outliers.to(self.device), 
                                                            self.objective)
                    shifting_const = 0.0
                    mean_module = InformativeMeanPrior(
                        self.train_y.to(self.device),
                ...
                self.model = RCaGP(
                    ...
                    train_targets=self.normed_train_y.to(self.device),
    ...
    def run(self):
        ...
        while self.objective.num_calls < self.max_n_oracle_calls:
            ...
            if self.exact_gp_baseline:
                ...
                if self.rcsvgp_baseline:
                    if self.informative_mean:
                        len_train_y = len(self.train_y)
                        condition = len_train_y - self.update_on_n_pts
                        idx = self.outliers_idx[self.outliers_idx >= condition] 
                        len_idx = len(idx)
                        ...
                        if len_idx > 0:
                            ...
                            idx = idx - condition
                            corrections = self.Y_corrections[-len_idx:]
                            y = self.train_y[-self.update_on_n_pts:]
                    ...
                    covar_module = self.model.covar_module
                    likelihood = self.model.likelihood.to(self.device)
                    projection_dim = len(normed_y_update_on)
            ...
            noise_y_next, noise_idx_y_next, noise_mask_y_next = self.objective.asymmetric_contamination(normed_y_next, probability=self.p_outliers)
            contaminated_normed_y_next = self.objective.inject_contamination(normed_y_next, self.normed_train_y_std, noise_y_next, noise_mask_y_next)
            
            # Update unnormalized contaminated and normed y
            revert_contaminated_y_next = contaminated_normed_y_next * self.train_y_std + self.train_y_mean
            self.contaminated_train_y = torch.cat((self.contaminated_train_y, revert_contaminated_y_next), dim=-2)
            self.normed_train_y = torch.cat((self.normed_train_y, contaminated_normed_y_next), dim=-2)
            ...
            if len(noise_idx_y_next) > 0:
                ...
                # Update corrections
                if self.informative_mean: 
                    correction = simulate_expert_correction(x_next, self.objective)
                    self.Y_corrections = torch.cat((self.Y_corrections, correction), dim=-2)
