    ...
    def _batch_call(self, x: torch.Tensor) -> MultivariateNormal:
        if self.training:
            ...
            if self.m_w is None:
                ...
                self.J_w = torch.diag(self.sigma_sq / 2 * 1 / (w ** 2))
            ...
            if self.cholfac_gram_SKhatS is None:
                ...
                reshape_J_w = (self.sigma_sq ) * self.J_w.diagonal().view(self.projection_dim, self.train_targets.size(-1) // self.projection_dim)
            ...
            covar = kxx - operators.RootLinearOperator(root=covar_x_train_actions_cholfac_inv)
            covar = torch.absolute(covar.evaluate())
            return mean, covar
