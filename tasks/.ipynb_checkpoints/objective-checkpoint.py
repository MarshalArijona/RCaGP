...
class Objective:
    ...
    def inject_contamination(self, ys, std, noise, mask, low=1.0, high=2.0):
        y_contaminated = ys.reshape(-1, 1) + mask.reshape(-1, 1) * (low * std + ((high - low) * std) * noise).reshape(-1, 1) 
        return y_contaminated

    def asymmetric_contamination(self, ys, seed=0, probability=0.1):
        n_data = ys.shape[0]
        random_tensor = torch.rand(n_data)
        mask = (random_tensor < probability).float()
        outliers_idx = torch.nonzero(mask, as_tuple=True)[0]

        torch.manual_seed(seed)

        #outliers_sample = 3 * std + (9 * std - 3 * std) * torch.rand(n_data)
        outliers_sample = torch.rand(n_data)
        #outliers = mask.reshape(-1, 1) * outliers_sample.reshape(-1, 1)
        #contaminated_ys = ys + outliers
        
        return outliers_sample, outliers_idx, mask
